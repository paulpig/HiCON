...
from utils.data_loader_entity import load_data
# from modules.MCCLK_v2 import Recommender
# from modules.MCCLK_v3_not_good import Recommender
from modules.HiCON import Recommender
...
def ctr_eval(model, data):
    ...
    while start < data.shape[0]:
        ...
        _, scores, _, _ = model(batch, is_training=False)
...
def topk_eval(model, train_data, data):
    ...
    # test_users = list(set(train_record.keys()))
    # test2_users = list(set(test_record.keys()))
    user_num = 100
...
if __name__ == '__main__':
    ...
    """build dataset"""
    # graph: mx.MultiDiGraph (contains u-i pairs), mat_list: adj mats based on different relations.
    train_cf, test_cf, user_dict, n_params, graph, metapath_graph_list, metapath_steps, mat_list, user_metpath_idxes, item_metpath_idxes = load_data(args)
    # pdb.set_trace()
    # norm_mat_list: adj mats based on different relations.
    ...
    # pdb.set_trace() # set(train_cf[:,0]) & set(test_cf[:,0]) # 存在重复;
    """cf data"""
    ...
    """define model""" # mean_mat_list[0] is a collaborative matrix.
    # model = Recommender(n_params, args, graph, metapath_graph_list, metapath_steps, mean_mat_list[0]).to(device)
    model = Recommender(n_params, args, graph, metapath_graph_list, metapath_steps, norm_mat_list[0], user_metapath_idxes=user_metpath_idxes, item_metapath_idxes=item_metpath_idxes).to(device)
    ...
    for epoch in range(args.epoch):
        ...
        while s + args.batch_size <= len(train_cf):
            ...
            batch_loss, _, _, _ = model(batch, is_training=True)
        ...
        if 1:
            ...
            # train_cf_pairs_np = np.array([[cf[0], cf[1], cf[2]] for cf in train_cf], np.int32)
            # test_cf_pairs_np = np.array([[cf[0], cf[1], cf[2]] for cf in test_cf], np.int32)
            # recall = topk_eval(model, train_cf_pairs_np, test_cf_pairs_np) # 明天调试一下代码, 让其支持recall.
            # # pdb.set_trace()
            # print(" ".join([str(item) for item in recall]))
            ...
            if cur_best_pre_auc < test_auc and cur_best_pre_f1 < test_f1:
                cur_best_pre_auc = test_auc
                cur_best_pre_f1 = test_f1
                cur_best_epoch = epoch
                early_stop_iter = 0
                print("best epoch: {}, auc: {}, f1: {}".format(cur_best_epoch, cur_best_pre_auc, cur_best_pre_f1))

                # save embeddings
                model_name = "CrossRecNew"
                dirs = './exp/music/{}/'.format(model_name)
                if not os.path.exists(dirs):
                    os.makedirs(dirs)
                user_all_embeddings = model.user_all_embeddings
                entity_all_embeddings = model.entity_all_embeddings

                np.save(dirs + 'user_emb', user_all_embeddings.cpu().detach().numpy())
                np.save(dirs + 'item_emb', entity_all_embeddings.cpu().detach().numpy())
            else:
                early_stop_iter += 1

            if early_stop_iter > 20:
                break
        print('early stopping at %d, test_auc:%.4f, test_f1:%.4f' % (cur_best_epoch, cur_best_pre_auc, cur_best_pre_f1))
